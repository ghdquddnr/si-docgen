...
**LLM 설정** 화면에서 로컬(Ollama)과 상용(OpenAI·Google Gemini·Anthropic·xAI Grok) 모델을 등록하고, 상용 API 키를 여러 개 저장·관리할 수 있습니다. 등록한 모델은 각 문서 생성 화면의 **"생성 모델"** 셀렉트박스에 나타납니다. 모든 LLM 호출은 LiteLLM 래퍼만 경유하며, 저장된 API 키는 `SIDOCGEN_SECRET_KEY`로 **암호화되어** 보관되고 화면·로그에는 마스킹된 미리보기만 노출됩니다.
...
[MIT License](LICENSE) — 저작권 고지를 유지하면 자유롭게 사용·수정·상업 이용할 수 있습니다.
