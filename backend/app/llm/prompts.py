...
INTERFACE_SPEC_SYSTEM = (
    "당신은 한국 SI 프로젝트의 시스템 연계(인터페이스) 설계 전문가다. "
    "요구사항 문서를 분석해 인터페이스정의서를 작성한다. "
    "반드시 지시된 JSON 스키마에 맞는 JSON 객체 하나만 출력한다."
)

INTERFACE_SPEC_PROMPT_TEMPLATE = """다음 원천 문서를 분석하여 인터페이스정의서를 작성하라.
...
[작성 규칙]
1. 출력은 아래 JSON 스키마를 만족하는 JSON 객체 하나만 출력한다. 설명 문장·마크다운 금지.
2. 시스템 간/대외기관 연계가 필요한 인터페이스를 도출한다. interface_id 는 IF-001 부터
   순번으로 부여한다 (중복 금지).
3. send_system(송신)·recv_system(수신) 시스템명을 명확히 적는다.
4. method(연계 방식)는 REST API/FTP/MQ/DB Link/Web Service 등에서, cycle(주기)은
   실시간/일 1회 배치/수시 등에서 적절히 정한다.
5. fields(메시지 항목)에는 주고받는 데이터 항목을 빠짐없이 나열한다. 항목명은 인터페이스
   안에서 유일하게, data_type 은 String(n)/Number/Date 등 구체적으로, required 는 필수면 true.
6. 모든 한국어 텍스트는 한국어로 작성한다.
7. 표지 정보: project_name="{project_name}", system_name="{system_name}",
   author="{author}", written_date="{written_date}".
...
USER_MANUAL_SYSTEM = (
    "당신은 한국 SI 프로젝트의 테크니컬 라이터다. "
    "요구사항 문서를 분석해 최종 사용자용 사용자 매뉴얼을 작성한다. "
    "반드시 지시된 JSON 스키마에 맞는 JSON 객체 하나만 출력한다."
)

USER_MANUAL_PROMPT_TEMPLATE = """다음 원천 문서를 분석하여 사용자 매뉴얼을 작성하라.
...
[작성 규칙]
1. 출력은 아래 JSON 스키마를 만족하는 JSON 객체 하나만 출력한다. 설명 문장·마크다운 금지.
2. 기능/메뉴 단위로 sections 를 나누고, 각 섹션에 title 과 간단한 description 을 적는다.
3. 각 단계(steps)의 instruction 은 최종 사용자가 따라 할 수 있게 구체적인 행동으로 쓴다
   (예: "[로그인] 버튼을 클릭합니다"). 한 단계 = 한 동작 단위.
4. screen_ref 에는 그 단계와 연관된 화면 ID(예: SCR-001)를 적는다. 화면을 특정하기 어려우면 "".
   (실제 화면 캡처 이미지는 이후 단계에서 screen_ref 기준으로 삽입되므로 이미지는 출력하지 않는다.)
5. caption 에는 화면 캡처에 붙일 짧은 설명을 적는다(선택).
6. 모든 텍스트는 한국어로, 정중한 안내문 어조로 작성한다.
7. 표지 정보: project_name="{project_name}", system_name="{system_name}",
   author="{author}", written_date="{written_date}".
...
def build_user_manual_prompt(
    ...
    *,
    ...
    """사용자 매뉴얼 생성 프롬프트를 조립한다 (이미지는 출력하지 않고 screen_ref 로만 참조)."""
    return USER_MANUAL_PROMPT_TEMPLATE.format(
        filename=source.filename,
        source_text=source_to_prompt_text(source),
        schema_json=schema_to_prompt_json(schema_cls),
        project_name=project_name,
...
def build_interface_spec_prompt(
    ...
    *,
    ...
    """인터페이스정의서 생성 프롬프트를 조립한다."""
    return INTERFACE_SPEC_PROMPT_TEMPLATE.format(
        filename=source.filename,
        source_text=source_to_prompt_text(source),
        schema_json=schema_to_prompt_json(schema_cls),
        project_name=project_name,
