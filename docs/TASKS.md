...
## C 시리즈 — 엔터프라이즈 제품화 (단계별 메뉴 + 셸 + 양식 보관함 + 제안서)

**배경**: 단순 포폴이 아니라 실제 배포·사용(GitHub 공개 + 도입 의향 회사 1곳). "거대 단일 파이프라인(캔버스)"을 SI 단계별 문서 메뉴로 재편하고, 엔터프라이즈 셸·양식 보관함·제안서를 추가한다.

**확정 결정(2026-06-16 사용자)**:
1. 캔버스(/canvas, 한 번에 전부 생성) **제거** → 문서별 독립 메뉴(업로드→생성→검수→다운로드).
2. **추적성**: 시나리오+화면+RTM 은 한 메뉴("테스트 설계")로 묶어 REQ→SCR→TC 유지(절대 원칙 6). WBS·테이블·인터페이스·매뉴얼은 독립 메뉴.
3. **레이아웃**: 좌측 사이드바(단계별 그룹) + 상단바(프로젝트 컨텍스트) 셸.
4. **홈**: 대시보드(문서 메뉴 카드 + 최근 생성 이력 표) — `GET /jobs` 목록 API 신규.
5. **비주얼**: 기존 slate+indigo 정제·고도화.
6. **양식 보관함**: A(트리/폴더 + 업로드 + 문서 생성 시 선택 + 없으면 기본) + B1(기본양식 꾸미기형 — 구조 보존, 업로드 시 필수 태그/헤더/shape 검증). 임의양식 자동매핑(B2, B7 분석기 연동·설정 구동 렌더러)은 **로드맵**. 로그인 없으니 배포 단위 단일 보관함.

> 산출물 메뉴 매핑: 제안서(RFP→PPTX·신규) / 요구사항정의서(원천→docx) / 테스트 설계(요건→화면+시나리오+RTM) / WBS / 테이블정의서 / 인터페이스정의서 / 사용자 매뉴얼. 입력은 기존 `load_source`(docx/pdf/md/txt)가 그대로 파싱.

### C1. 백엔드 산출물 독립화 ⚠️ (메뉴화 토대)
- [x] `run_job` 을 **요청한 산출물만 생성**하도록 재구성 — 무조건 시나리오를 만들던 `else` 제거. `with_requirements`=요건정의서만, `with_screens`=테스트 묶음(시나리오+화면), 나머지(wbs/table/interface/manual)는 각자 독립. 각 with_* 블록 분리.
- [x] `render_job_outputs(scenario_json 기본 None)` — 시나리오 없이도 개별 산출물 단독 렌더. RTM·test_scenario 는 시나리오 있을 때만, 화면정의서는 함께 있으면 추적성 연결. requirement_spec 은 독립 docx(시나리오 동반 시에만 교차검증).
- [x] `GET /jobs?limit=` 목록 API(대시보드 최근 이력용, 생성일 내림차순) + render 409 조건을 "시나리오 없음"→"산출물 없음"으로 변경.
- **AC**: 단일 산출물 잡(WBS만 등)이 시나리오 없이 해당 문서만 생성, 테스트 묶음(with_screens)은 화면+시나리오+RTM 추적성 유지. 기존 e2e 6파일 갱신 + 목록 테스트 신규. **286 통과, ruff 클린.**
...
- [x] **웹 연동(C4-3)**: `Job.client`/`with_proposal`/`proposal_json`/`proposal_model` 컬럼 + 마이그레이션(c4f1a2b3d5e6), `run_job` 제안서 분기(progress "proposal", client 주입), `render_job_outputs(proposal_json=...)` 제안서 렌더, download kind `proposal`(pptx), `GET/PUT /jobs/{id}/proposal`, `templates_service` 에 proposal(.pptx) 추가. 프론트: `menus.ts` 제안서 `available:true`·`needsClient`, `GenerateFlow` 발주처 필드(제안사 라벨), `api.ts` Proposal 타입·get/putProposal, `ProposalEditor`, **검수 페이지 비-시나리오 산출물 대응**(scenario 옵셔널화).
- **AC**: RFP → 제안서 pptx 초안. 모킹 e2e + eval. 양식 사람 게이트.
- 메모(C4-2): 표준 8섹션(①사업이해 ②추진전략 ③수행방안 ④추진일정 ⑤투입조직 ⑥품질·보안 ⑦기대효과 ⑧결론·제언)을 프롬프트로 권장. **목차는 프롬프트에서 명시적으로 LLM 출력 제외**(렌더러 자동 생성과 중복 방지). 제안서는 `client`(발주처) 표지 항목이 추가로 필요해 cover 4종 + `client`. CLI/eval 포함, 총 310 통과.
- 메모(C4-3): **검수 페이지가 그동안 scenario 를 강제 로드**(WBS·테이블·인터페이스 단독 잡도 막혔던 잠재 버그)했던 것을 일반화 — `getScenario` 는 `with_screens` 일 때만, 첫 탭은 잡이 가진 산출물에서 자동 선택, 편집 탭 없는 잡(WBS 등)은 "렌더링·다운로드만" 패널 노출. 헤더는 `scenario.*`→`job.*`. 발주처는 `Job.client` 신규 컬럼(다른 산출물은 빈 값). run_job 의 client 미지정 시 "발주처" 폴백. **백엔드 316 통과(제안서 API e2e 6건)·ruff 클린, 프론트 lint/build 통과, 마이그레이션 dev DB 적용 확인, 프리뷰에서 `/generate/proposal` 폼(제안사·발주처·양식 피커) 정상.** 다음: C5 통합 게이트.

### C5. C 시리즈 통합 사람 검수 게이트
- [x] 대시보드~메뉴별 생성~양식 선택~제안서 전 과정 실브라우저 판정.
- 메모: **2026-06-16 사용자 검수 — 합격.** 프리뷰 점검(콘솔 에러 0): 대시보드 최근 이력(제안서 라벨 포함)·`/generate/proposal`(제안사·발주처·양식 피커)·`/generate/test-design`(작성자, 발주처 누수 없음)·제안서 검수→렌더→다운로드(pptx 6슬라이드=표지+목차자동+본문4) 정상. 빌드/린트/타입체크 통과, 백엔드 316 통과. 부수 수정: 대시보드 제안서 라벨(cce1226). 검수용 시드 잡은 게이트 후 삭제. **C 시리즈 종료.**
...
### B7-1. 엑셀 양식 분석기
- [x] `backend/app/onboarding.py` — 고객 xlsx 양식을 읽어 **표지 셀·헤더 행·서식 기준 행·컬럼 매핑**을 휴리스틱 제안(LLM·네트워크 없음). CLI `analyze-template --input 고객.xlsx --kind ...`.
- **AC**: 우리 템플릿 자체 분석으로 정확 매핑 + 종류 불일치/미지원/없는 파일 처리. → 통과(9건, 총 275건).
- 메모: **사람 검수용 제안 도구**(완전 자동 아님). 표지 라벨(프로젝트명/시스템명/작성자/작성일+동의어) → 오른쪽 병합 해소 값 셀 추정. 헤더 행 = 기대 라벨 최다 매칭 행, 서식 기준 행 = 헤더+1. 컬럼은 느슨 매칭(정규화 동일/포함). 지원 종류: test_scenario/wbs/table_spec/interface_spec(단일 헤더). RTM 다단 헤더·docx/pptx 는 범위 밖. 출력은 보고서 문자열 + `to_dict`(JSON). **렌더러를 설정 구동으로 바꾸는 건 후속**(현재는 위치 제안까지 = 백로그 항목 '플레이스홀더 위치 제안 도구' 충족).
