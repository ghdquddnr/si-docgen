...
- **현재 Phase**: 로드맵(Phase 0~4) 완료. **백로그 작업 진행 중 — 요구사항정의서(docx) 생성**.
- **진행 중 태스크**: B1-2 (요건정의서를 체인의 머리로 연결). B1-1(단독 생성) 완료.
- **차단 사항**: 없음.
...
### B1-2. 체인의 머리로 연결 (진행 중)
- [ ] 요건정의서를 체인 선두에 배치 — 확정 REQ ID 를 시나리오·화면정의서 생성에 주입, RTM 요건명을 요건정의서에서 채움
- **AC**: 미정 (착수 시 설계 선택지 제시).
