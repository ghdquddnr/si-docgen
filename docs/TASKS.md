...
- **진행 중 태스크**: P0-10 (사람 검수 게이트 — 검수용 샘플은 `out/review/` 에 생성됨)
- **차단 사항**: P0-10 은 사용자 판정 필요 (자동화 불가)
...
- 메모: 검수용 샘플 3종 생성 완료 → `out/review/` (테스트시나리오_샘플.xlsx / 요구사항정의서_샘플.docx / 화면정의서_샘플.pptx). PowerPoint·LibreOffice 실제 열림 확인(P0-8 AC 잔여 항목)도 이 검수에서 함께 수행할 것. 재생성: `uv run python scripts/templates/build_*.py` 후 렌더러 실행.
