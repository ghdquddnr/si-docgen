"""인터페이스정의서 엑셀 양식(backend/templates/interface_spec.xlsx) 제작 스크립트.

템플릿을 재현 가능하게 만드는 일회성 도구이며, 런타임 렌더러는 생성된 .xlsx 를 사용한다.
단일 시트 '인터페이스정의서', 11열 단일 헤더(목록형) + 표지/결재란 + 서식 기준 행.

실행: uv run python scripts/templates/build_interface_spec_template.py
"""
...
OUTPUT_PATH = Path(__file__).resolve().parents[2] / "backend" / "templates" / "interface_spec.xlsx"
...
# 좌측 정렬(줄바꿈)할 열: 인터페이스명·송수신 시스템·항목명·설명
LEFT_COLUMNS = {3, 4, 5, 8, 11}
